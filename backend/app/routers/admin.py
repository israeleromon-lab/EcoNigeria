...
from app.services.etl.runner import run_all_etl
import asyncio
...
async def trigger_etl(background_tasks: BackgroundTasks):
    ...
    """
    try:
        # Define a wrapper to run the synchronous run_all_etl in a background thread if needed,
        # but since run_all_etl is currently written as a standard function, we can just pass it.
        background_tasks.add_task(run_all_etl)

...
export function EconomicSignalsBanner() {
  ...
  return (
    <div className="border-4 border-foreground bg-background p-5 shadow-sm">
      {/* Header bar */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b-2 border-foreground pb-3">
        ...
        <div className="flex items-center gap-2">
          {criticalCount > 0 && (
            <span className="px-2 py-0.5 text-[11px] font-mono font-bold uppercase tracking-wider bg-destructive/10 text-destructive border border-destructive/30">
              {criticalCount} Critical
            </span>
          )}
          {warningCount > 0 && (
            <span className="px-2 py-0.5 text-[11px] font-mono font-bold uppercase tracking-wider bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-500/30">
              {warningCount} Warning
            </span>
          )}
          {positiveCount > 0 && (
            <span className="px-2 py-0.5 text-[11px] font-mono font-bold uppercase tracking-wider bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-500/30">
              {positiveCount} Stabilizer
            </span>
          )}
          <button
            onClick={() => setIsExpanded(!isExpanded)}
            className="flex items-center gap-1 text-xs font-mono font-bold uppercase tracking-wider hover:bg-muted px-2 py-1 border border-border transition-colors ml-2"
          >
            {isExpanded ? (
              <>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-serif text-muted-foreground mt-1 leading-relaxed">
          ...
          <div className="shrink-0 flex lg:flex-col items-baseline lg:items-end justify-between border-t lg:border-t-0 lg:border-l border-border pt-2 lg:pt-0 lg:pl-6">
        ...
        <div className="mt-6 pt-6 border-t border-border grid grid-cols-1 md:grid-cols-2 gap-4">
          {signals.slice(1).map((sig) => {
            ...
            return (
              <div
                key={sig.id}
                className={`p-4 border bg-muted/20 flex flex-col justify-between ${style.border}`}
              >
                <div>
      ...
      <div className="mt-4 pt-3 border-t border-border flex items-center justify-between text-xs font-mono text-muted-foreground">
        ...
        <Link 
          href="/research" 
          className="hover:text-foreground inline-flex items-center gap-1 font-bold underline underline-offset-4"
        >
